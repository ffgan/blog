...
初步在本机上编译出来了这几个官方支持的架构的docker image，比较简单，make all就结束了。本来想照猫画虎直接加一下riscv64看能否也编译出来。果断吃一个报错。通过分析其makefile可知，这个构建环境是被打包进了一个包含多个平台的编译工具的image里面，直接进入该image里面，检索发现并不存在riscv对应的编译工具。这下就比较麻烦了，需要琢磨之前是如何处理对于新架构的工具链支持。感觉大概率和bazel有关。
